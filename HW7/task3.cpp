...
int main()
{
    ...
    for (unsigned long i{0}; i < img_files.size()-1; i++)
    {
...
#ifdef SHOW
    cv::imshow("First Image", img(roi_can));
    cv::waitKey(0);
#endif
}
...
void calcTimeToImpact(std::vector<cv::Point2d>& prev_features, std::vector<cv::Point2d>& features)
{
    static int count{0};
    double z{0};
    double X{59}, focal{825};

    if (features.size() > 1)
    {
        double dx{features[1].x - features[0].x};
        dx *= (dx < 0) ? -1 : 1;
        z = focal * X / dx;
